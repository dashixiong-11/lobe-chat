...
const getStreamString = (type: keyof typeof StreamStringPrefixes, value: JSONValue): StreamString =>
  `${StreamStringPrefixes[type]}:${JSON.stringify(value)}\n`;
...
function createFunctionCallTransformer(
  ...
  let functionCallMessages: CreateMessage[] = callbacks[__internal__OpenAIFnMessagesSymbol] || [];
...
async function* streamable(
  stream: AsyncIterableOpenAIStreamReturnTypes,
  cb?: (chunk: ChatCompletionChunk | Completion) => void,
) {
  const extract = chunkToText();
